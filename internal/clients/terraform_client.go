...
func TerraformSetupBuilder(version, providerSource, providerVersion string) terraform.SetupFn {
	return func(ctx context.Context, client client.Client, mg resource.Managed) (terraform.Setup, error) {
		...
		if err := json.Unmarshal(data, &clientCreds); err != nil {
		...
		/*ps.Env = []string{
			fmt.Sprintf("%s=%s", "HASHICUPS_USERNAME", clientCreds["username"]),
			fmt.Sprintf("%s=%s", "HASHICUPS_PASSWORD", clientCreds["password"]),
		}*/
		...
		if v, ok := clientCreds["organization"]; ok {
			ps.Configuration["organization"] = v
		}
		
		if v, ok := clientCreds["owner"]; ok {
			ps.Configuration["owner"] = v
		}
		
		if v, ok := clientCreds["token"]; ok {
			ps.Configuration["token"] = v
		}
		
		if v, ok := clientCreds["write_delay_ms"]; ok {
			ps.Configuration["write_delay_ms"] = v
		}
		
		if v, ok := clientCreds["base_url"]; ok {
			ps.Configuration["base_url"] = v
		}
		
		if v, ok := clientCreds["insecure"]; ok {
